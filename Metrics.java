...
public class Metrics {
    ...
    public void script(String taxonID, String taxon, boolean printGraphs, String queryPeriodo, String tabela) {
        ...
        db.setDBName("<database>");              //change database name if needed
        db.setHost("<host_name>");                //add name of mysql server
        db.setUsername("<username>");             //change database username
        db.setPasswd("<password>");               //change database password
        ...
        //debug 
        //SELECT concat(v.Nodes, '-',taxonid) as Nodes, concat(v.Nodes, '-',taxonid) AS Id, v.Nodes AS Label, latitude, longitude, taxonid FROM vertices1Species_wild v WHERE taxonid = 1 AND v.Nodes <> 'XX' AND year >= 2006 GROUP BY v.Nodes, taxonid
        
        //SELECT concat(exporter,'-',taxonid) AS Source, concat(importer,'-',taxonid) AS Target, 'Directed' AS type, CONCAT(exporter,'-',importer) AS Label, quant AS weight, reexport, log(quant+1)*10 AS logquant, quant, n_taxon, taxonid FROM edgesSpecies_wild WHERE taxonid = 1 AND exporter <> 'XX' AND year >= 2006 ORDER BY taxonid;
...
//            System.err.println("Database connection established");
        ...
        for (Node node : graph.getNodes().toArray()) {
            ...
            String query = "INSERT INTO " + db.getDBName() + ".metrics"+tabela+"_wild "
