...
public class MetricsEU {
    ...
    public void script(String taxonID, String taxon) {
        ...
        db.setNodeQuery("SELECT concat(v.Nodes, '-',taxonid) as Nodes, concat(v.Nodes, '-',taxonid) AS Id, "
                + "v.Nodes AS Label, taxonid "
                + "FROM vertices1Species v WHERE taxonid = "+taxonID+" AND v.Nodes <> 'XX' "
                + "GROUP BY v.Nodes, taxonid");
        db.setEdgeQuery("SELECT concat(exporter,'-',taxonid) AS Source, concat(importer,'-',taxonid) AS Target, "
                ...
                + "FROM edgesSpecies WHERE taxonid = "+taxonID+" AND exporter <> 'XX' AND "
                + "importer IN ('BE','BG','CZ','DK','DE','EE','IE','GR','ES','FR','IT','CY','LV','LT','LU','HU','MT','NL','AT','PL','PT','RO','SI','SK','FI','SE','GB') "
                + "ORDER BY taxon;");
        ...
        prv.script(container, taxonID+"_"+taxon);
...
//            System.err.println("Database connection established");
        ...
        for (Node node : graph.getNodes().toArray()) {
            ...
            Double eccent = (Double) node.getNodeData().getAttributes().getValue(col[4].getId());
            Double closeCentral = (Double) node.getNodeData().getAttributes().getValue(col[5].getId());
            Double betCentral = (Double) node.getNodeData().getAttributes().getValue(col[6].getId());
            Integer inDegree = (Integer) node.getNodeData().getAttributes().getValue(col[7].getId());
            Integer outDegree = (Integer) node.getNodeData().getAttributes().getValue(col[8].getId());
            Integer degreeVal = (Integer) node.getNodeData().getAttributes().getValue(col[9].getId());
            Integer modular = (Integer) node.getNodeData().getAttributes().getValue(col[10].getId());
            Double pagerankVal = (Double) node.getNodeData().getAttributes().getValue(col[11].getId());
            Float authority = (Float) node.getNodeData().getAttributes().getValue(col[12].getId());
            Float hub = (Float) node.getNodeData().getAttributes().getValue(col[13].getId());
            Double eigenCentral = (Double) node.getNodeData().getAttributes().getValue(col[14].getId());
            Object clustCoef = node.getNodeData().getAttributes().getValue(col[15].getId());
            Integer componentID = (Integer) node.getNodeData().getAttributes().getValue(col[16].getId());
            Integer strongConnectID = (Integer) node.getNodeData().getAttributes().getValue(col[17].getId());
            
                    

            String query = "INSERT INTO " + db.getDBName() + ".metricsEU "
