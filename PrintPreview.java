...
import org.gephi.plugins.layout.geo.GeoLayoutBuilder;
...
public class PrintPreview {
    ...
    public void script(Container container, String filename) {
...
//        Container container;
        try {
//            File file = new File(getClass().getResource("/tradesps/polblogs.gml").toURI());
...
//        }
        ...
        AttributeColumn latitude = attributeModel.getNodeTable().getColumn("latitude");
        AttributeColumn longitude = attributeModel.getNodeTable().getColumn("longitude");
        
        GeoLayoutBuilder glb = new GeoLayoutBuilder();
        GeoLayout layout = new GeoLayout(glb);
        layout.setGraphModel(graphModel);
//        layout.resetPropertiesValues();
        layout.setLatitude(latitude);
        layout.setLongitude(longitude);
        layout.initAlgo();
        for (int i = 0; i < 100 && layout.canAlgo(); i++) {
            layout.goAlgo();
        }
...
//        model.getProperties().putValue(PreviewProperty.EDGE_RESCALE_WEIGHT, Boolean.TRUE);
        model.getProperties().putValue(PreviewProperty.EDGE_THICKNESS, new Float(0.001f));
